...
REPO_DIR="$HOME/time-server"
MON_DIR="$REPO_DIR/monitoring"
SYSTEMD_DIR="$REPO_DIR/systemd"
REBUILD_DIR="$REPO_DIR/rebuild"
TIMING_DIR="$HOME/timing"

mkdir -p "$MON_DIR" "$SYSTEMD_DIR" "$REBUILD_DIR" "$TIMING_DIR"

cat > "$MON_DIR/zed_monitor.py" <<'PY'
#!/usr/bin/env python3
import json
import math
import os
...
chmod +x "$MON_DIR/zed_monitor.py"

cat > "$SYSTEMD_DIR/zed-monitor.service" <<'SERVICE'
[Unit]
Description=ZED-F9T monitor via gpsd JSON
After=network-online.target gpsd-direct.service zed-splitter.service
...
User=pi
Group=pi
WorkingDirectory=/home/pi/time-server
Environment=PYTHONUNBUFFERED=1
Environment=ZED_MONITOR_DB=/home/pi/timing/zed_monitor.db
Environment=ZED_MONITOR_STATUS=/home/pi/timing/zed_status.json
Environment=ZED_MONITOR_POLL_SECONDS=10
ExecStart=/usr/bin/python3 /home/pi/time-server/monitoring/zed_monitor.py
Restart=always
RestartSec=5
...
cat > "$REBUILD_DIR/install_zed_monitor.sh" <<'INSTALL'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x "$REBUILD_DIR/install_zed_monitor.sh"

if [ -f "$REPO_DIR/rebuild/dump_state.sh" ]; then
  python3 - <<'PY'
from pathlib import Path
p = Path.home() / 'time-server' / 'rebuild' / 'dump_state.sh'
text = p.read_text()
block = r'''echo "[ ZED MONITOR ]"
if systemctl is-active --quiet zed-monitor.service; then
  echo "OK    zed-monitor.service running"
else
  echo "WARN  zed-monitor.service not running"
fi

if [ -f /home/pi/timing/zed_status.json ]; then
  echo "Latest ZED status:"
  python3 - <<'PY2'
import json
p = "/home/pi/timing/zed_status.json"
with open(p, "r", encoding="utf-8") as f:
    d = json.load(f)
print(f"  ts={d.get('ts_utc')} fix={d.get('fix_type')} used={d.get('sats_used')} visible={d.get('sats_visible')} pdop={d.get('pdop')} avg_cn0={d.get('avg_cn0')}")
PY2
else
  echo "WARN  /home/pi/timing/zed_status.json missing"
fi

echo
'''
if '[ ZED MONITOR ]' not in text:
    marker = '\necho "[ GIT ]"\n'
    if marker in text:
        text = text.replace(marker, '\n' + block + marker, 1)
        p.write_text(text)
        print('Patched dump_state.sh with [ ZED MONITOR ] section')
    else:
        print('Did not patch dump_state.sh automatically: [ GIT ] marker not found')
else:
    print('dump_state.sh already contains [ ZED MONITOR ] section')
PY
fi

echo
echo "Files created:"
echo "  $MON_DIR/zed_monitor.py"
echo "  $SYSTEMD_DIR/zed-monitor.service"
echo "  $REBUILD_DIR/install_zed_monitor.sh"
echo
echo "Next:"
echo "  cd ~/time-server"
echo "  ./rebuild/install_zed_monitor.sh"
echo "  journalctl -u zed-monitor.service -n 20 --no-pager"
echo "  cat /home/pi/timing/zed_status.json"
